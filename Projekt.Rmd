...
=======
I detta arbete hardvi använt oss utav tidsseriedata som hämtats från SCB. 
>>>>>>> Stashed changes
...
min_lista <- list(Region=px_data_frame_befolkning$region, Medelålder=px_data_frame_age$`Befolkningens medelålder`,  Inkomst=px_data_frame_income$Medelvärde, "Utländska.procent"=100*antalUtland/px_data_frame_befolkning$Folkmängd, "MopparPer100Personer"=100*iTrafik$`Fordon i trafik`/px_data_frame_befolkning$Folkmängd, Befolkning=px_data_frame_befolkning$Folkmängd)
...
knitr::kable(total_df[150:154,], caption="Tabell för kommundata")
```
...
I denna tabell har vi data för medelålder, medellön på årsbasis, andel utländska, antal moppar per 100 personer samt antalet invånare.


```{r medel, echo=FALSE}
medel.age <- mean(total_df$Medelålder)
medel.utland <- mean(total_df$"Utländska.procent")
medel.inkomst <- mean(total_df$Inkomst)
medel.moppar <- mean(total_df$"MopparPer100Personer")
...
median.utland <- median(total_df$"Utländska.procent")
median.moppar <- median(total_df$"MopparPer100Personer")
...
standardavikelse.utland <- sd(total_df$"Utländska.procent")
standardavikelse.moppar <- sd(total_df$"MopparPer100Personer")
...
lowest.utland <- min(total_df$"Utländska.procent")
lowest.moppar <- min(total_df$"MopparPer100Personer")
...
highest.utland <- max(total_df$"Utländska.procent")
highest.moppar <- max(total_df$"MopparPer100Personer")
...
names_table2 <- c("Medelålder", "Medelinkomst", "Utländska", "Moppar", "Befolkning")
...
knitr::kable(table2_dataframe,caption = "Test",digits = 3, format.args = list(big.mark = ",",
  scientific = FALSE))
```
...
I denna tabell till skillnad från den ovanför så är representerar variablerna Sveriges samtliga regioner, 
inte enbart våra utvalda kommuner
```{r plot, echo=FALSE}
plot(px_data_frame_income$Medelvärde, 
     100*iTrafik$`Fordon i trafik`/px_data_frame_befolkning$Folkmängd,
     main="Scatterplot inkomst och moppar",
     xlab="Medelinkomst i tkr",
     ylab="Moppar per 100 pers",
     xlim=c(400, 1050))

```


En scatter plot mellan variablerna medelinkomst och antal moppar per 100 personer.
Det tycks inte föreligga någon signifikant korrelation mellan variablerna. 
Kommunen Danderyd utslöts ur plotten då dess medelinkomst låg nästan det dubbla 
av kommunen med näst högst inkomst.

```{r age.plot, echo=FALSE}

hist(result_age, main="Medelålder", ylab="Frekvens", xlab="Ålder", breaks=40)

```

```{r income.hist, echo=FALSE}

hist(px_data_frame_income$`Medelvärde,  tkr`, xlim =c(300, 1050), breaks=30, main="Medelinkomst för kommuner", xlab="Inkomst", ylab="Frekvens")

```

Data på medelinkomst för alla kommuner. 
Även här valt att exkludera Danderyd som annars ger ett fult histogram.


```{r utlandBoxplot, echo=FALSE}
boxplot(total_df$"Utländska.procent",range=0,horizontal=TRUE,xlab="Antal utlandsfödda per 100 invånare",col="blue")
```
Ovan visar en boxplot över antalet utlandsfödda per 100 invånare över samtliga regioner i Sverige

```{r box, echo=FALSE}

boxplot(100*iTrafik$`Fordon i trafik`/px_data_frame_befolkning$Folkmängd, horizontal = TRUE, main="Boxplot andel moppar per 100 invånare", col="blue", outline=FALSE, xlab="Moppar per 100")

```

Ovan illustreras en boxplot på andel moppar i kommunerna per 100 invånare. Vi
finner här en median strax över 1 moped per 100 invånare. I boxplotten har vi
tagit beslutet att ignorera outliers då detta annars starkt förvränger figuren.
